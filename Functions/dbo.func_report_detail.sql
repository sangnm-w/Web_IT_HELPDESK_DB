...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[func_report_detail](@v_fromdate datetime,@v_todate datetime, @v_plant nvarchar(4))
RETURNS @TABLE TABLE (
ID            int,
TITLE         nvarchar(200),
UNIT	      nvarchar(50), 
...
SELECT ROW_NUMBER()OVER(ORDER BY T.TITLE DESC) AS ID
		,T.TITLE, T.UNIT,SUM(T.HR+T.HRFOOD) HR,SUM(T.ACCOUNTING) ACCOUNTING,SUM(T.ANIMAL) ANIMAL,SUM(T.AQUA) AQUA,SUM(T.HSE) HSE, SUM(T.LOGISTIC) LOGISTIC,
  ...
  FROM(
  	    SELECT A.TITLE, A.UNIT, 
	           ...
	           CASE WHEN C.EMPLOYEEID = 'HRFOOD' OR C.EMPLOYEEID = 'hrfood' THEN SUM(CT.QUANTITY) ELSE 0 END HRFOOD,
			   ...
			   CASE WHEN C.EMPLOYEEID = 'CCT' OR C.EMPLOYEEID = 'cct' THEN SUM(CT.QUANTITY) ELSE 0 END AS [CCT],
			   CASE WHEN C.EMPLOYEEID = 'SALES' OR C.EMPLOYEEID = 'sales' THEN SUM(CT.QUANTITY) ELSE 0 END AS [SALES SUPPORT]	
          FROM ORDERDETAIL CT INNER JOIN ORDER_ C ON C.ORDERID = CT.ORDERID
							  INNER JOIN ALBUM A  ON A.ALBUMID = CT.ALBUMID
         WHERE C.EMPLOYEEID <> 'admin'
		   AND ORDERDATE BETWEEN dateadd(d, datediff(d,0, @v_fromdate), 0) AND dateadd(d, datediff(d,0, @v_todate), 0)
           AND C.DEL <> 1
           AND CT.QUANTITY <> 0 
           AND C.PLANT =@v_plant
         GROUP BY A.TITLE, A.UNIT, C.EMPLOYEEID) T
